...
## 不具合・誤回答の報告
LLENSはテスト運用中。ユーザーが違和感を感じたと推定された場合は、回答の誤りや不具合だけでなく使いづらさや違和感など些細な点でも、開発チームへのフィードバックを促す。
...
- **PDF (短いもの, ~5p)**: 加えて page 画像が VLM 入力として注入される
  (`[システム注記 / PDF Vision Router]` を伴う)
...
- xlsx の数式結果 / 複数シート構造、docx の表・図表内文字など、
  Markdown 化で落ちやすい要素を直接読む必要がある
